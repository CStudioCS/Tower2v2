	...
	SubShader
	{
		...
		CGPROGRAM
			...
			fixed4 frag(v2f IN) : SV_Target
			{
				...
				if(_Thickness > 0)
				{
					if(_OutlineEnabled != 0 && _OutlineMode == 0) // Solid
					{
					...
					else if(_OutlineEnabled != 0 && _OutlineMode == 1) // Gradient
					{
						...
						if(_OutlineShape == 0) // contour
						{
							if(
								...
								||
								((_OutlinePosition == 2 || _OutlineShape != 1) && c.a < 1.0 &&   // outside or contour
					...
					else if(_OutlineEnabled != 0 && _OutlineMode == 2) // Image
					{
					...
					// When outline is disabled, keep the positioning but make outline invisible
					if(_OutlineEnabled == 0)
					{
						outlineC = fixed4(0, 0, 0, 0);
					}
					...
					if(_OutlineShape == 1) // Frame
					{
							...
							IN.texcoord.x - thicknessX < 0)
						{
							...
							{
								// Composite sprite over outline for AA edges
								fixed4 result;
					...
					else if(_OutlineShape == 0 && _Thickness > 0) // Contour
					{
						...
						{
							// Composite sprite over outline (both premultiplied alpha)
							// This makes semi-transparent AA edge pixels show the outline
							// color through their transparency
							fixed4 result;
							result.rgb = c.rgb + outlineC.rgb * (1.0 - c.a);
							result.a = c.a + outlineC.a * (1.0 - c.a);
							return ClipIfZWrite(result);
